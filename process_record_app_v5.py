...
st.set_page_config(
    page_title="精神看護論実習 プロセスレコード練習アプリ v5",
    page_icon="📝",
    layout="wide",
)
...
def build_by_turn_text(turn_records, key, heading_prefix):
    blocks = []
    for rec in turn_records:
        content = (rec.get(key) or "").strip()
        if content:
            blocks.append(f"会話{rec['turn']}\n{content}")
        else:
            blocks.append(f"会話{rec['turn']}\n")
    return "\n\n".join(blocks)
...
st.caption("v5 修正版：基本情報の「実習日」を「記入日」に変更しました。")
...
for log in selected_logs:
    ...
    st.write(f"患者：{log['patient']}")
    st.write(f"学生：{log['student']}")
    st.write(f"患者：{log['reaction']}")
...
for log in selected_logs:
    ...
    st.markdown(f"### 会話{i}の記録")
    with st.container(border=True):
        if log["situation"]:
            st.write(f"**患者の様子：** {log['situation']}")
        st.write(f"**患者の発言：** {log['patient']}")
        st.write(f"**学生の返答：** {log['student']}")
        st.write(f"**患者の反応：** {log['reaction']}")

        patient_behavior = st.text_area(
            f"3-{i}．対象者の言動／様子（会話{i}における対象者の言葉、表情、視線、声の大きさ、姿勢、沈黙などを具体的に書いてください）",
            value=(
        ...
        student_feeling = st.text_area(
            f"4-{i}．学生が感じたこと／考えたこと（会話{i}の時、自分が感じたこと、考えたこと、戸惑ったこと、迷ったことを書いてください）",
            height=130,
            key=f"student_feeling_{scene_name}_{i}_{choice_key}",
        )
        student_action = st.text_area(
            f"5-{i}．学生の言動（会話{i}で自分が実際にどのように声をかけたか、どのような態度で関わったかを書いてください）",
            value=f"学生の返答：{log['student']}",
            height=110,
            key=f"student_action_{scene_name}_{i}_{choice_key}",
        )
        analysis_record = st.text_area(
            f"6-{i}．分析・考察（会話{i}について、対象者の言動の背景、自分の関わりが対象者に与えた影響、別の関わり方の可能性について考えてください）",
            height=150,
