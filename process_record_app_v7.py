...
st.set_page_config(
    page_title="精神看護論実習 プロセスレコード練習アプリ v7",
    page_icon="📝",
    layout="wide",
)
...
st.caption("v7 修正版：患者→学生→患者の発言ごとに通し番号を表示します。")
...
with st.expander("使い方", expanded=False):
    st.write(
        ...
        "6. 患者→学生→患者の発言ごとの通し番号を確認し、最後にWordファイルをダウンロードします。"
    )
    ...
    selected_logs.append(
        {
            ...
            "student_no": circled_number((i - 1) * 3 + 2),
            "reaction_no": circled_number((i - 1) * 3 + 3),
        }
    )
    st.divider()
...
st.write("発言の順序が分かるように、患者→学生→患者の各発言に通し番号を付けています。")
for log in selected_logs:
    st.markdown(f"**会話{log['turn']}**")
    if log["situation"]:
        st.write(f"患者の様子：{log['situation']}")
    st.write(f"{log['patient_no']} 患者：{log['patient']}")
    st.write(f"{log['student_no']} 学生：{log['student']}")
    st.write(f"{log['reaction_no']} 患者：{log['reaction']}")

st.header("プロセスレコードを記入してください")
st.write("各項目の説明は、入力欄の中ではなく項目名の横に表示しています。")
st.info("患者→学生→患者の発言ごとに、①②③…の通し番号を付けています。Word出力でも同じ通し番号が反映されます。")
...
for log in selected_logs:
    ...
    sequence_label = f"会話{i}（{log['patient_no']}患者→{log['student_no']}学生→{log['reaction_no']}患者）"
    st.markdown(f"### 会話{i}の記録（{log['patient_no']}患者→{log['student_no']}学生→{log['reaction_no']}患者）")
    with st.container(border=True):
        if log["situation"]:
            st.write(f"**患者の様子：** {log['situation']}")
        st.write(f"**{log['patient_no']} 患者の発言：** {log['patient']}")
        st.write(f"**{log['student_no']} 学生の返答：** {log['student']}")
        st.write(f"**{log['reaction_no']} 患者の反応：** {log['reaction']}")

        patient_behavior = st.text_area(
            f"3-{i}．対象者の言動／様子（{log['patient_no']}患者の発言・様子、{log['reaction_no']}患者の反応・様子を具体的に書いてください）",
            value=(
                (f"患者の様子：{log['situation']}\n" if log["situation"] else "")
                + f"{log['patient_no']} 患者の発言：{log['patient']}\n{log['reaction_no']} 患者の反応：{log['reaction']}"
            ),
            height=130,
            key=f"patient_behavior_{scene_name}_{i}_{choice_key}",
        )
        student_feeling = st.text_area(
            f"4-{i}．学生が感じたこと／考えたこと（{log['patient_no']}患者の発言、{log['student_no']}自分の返答、{log['reaction_no']}患者の反応それぞれについて感じたこと・考えたことを書いてください）",
            height=130,
        ...
        student_action = st.text_area(
            ...
            value=f"{log['student_no']} 学生の返答：{log['student']}",
            height=110,
            key=f"student_action_{scene_name}_{i}_{choice_key}",
        )
        analysis_record = st.text_area(
            f"6-{i}．分析・考察（{log['patient_no']}→{log['student_no']}→{log['reaction_no']}の流れから、対象者の言動の背景や自分の関わりの影響を考えてください）",
            height=150,
